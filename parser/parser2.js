...
var NLParser2 = {
    ...
    // Namespace object
    parserFactories: {"en": () => new NLParser2.Parser({
            lang: "en",
...

{
    ...
    NLParser2.Parser = function(props) {
    ...
    NLParser2.Parser.prototype = {
        ...
        addCommandToList: function addCommandToList(command) {
            if (command.oldAPI && !gOldAlerted) {
                CmdUtils.notify("A verb was not loaded " +
...
//                "extensions.ubiquity.doNounFirstExternals", 0);
            ...
            this.extraArgsToDetect = Object.keys(nounIdsForGivenInputs).map(input =>
                ({argText: input, nounTypeIds: nounIdsForGivenInputs[input]}));

            for (let nt of Object.values(this._nounTypes)) {
        ...
        initializeCache: function initializeCache() {
            ...
            for (let {role, delimiter} of this.roles)
            ...
            for (let verb of verbs) {
                let subsets = Utils.powerSet(verb.arguments.map(arg => arg.role));
            ...
            function regexFromDelimeters(roles) {
                return RegExp("^" + RegexpTrie(roles.map(role => role.delimiter)) + "$",
                    "i")
            }
            ...
            for (let verb of verbs) {
                ...
                rolesCache[verb.id] = this.roles.filter(role => verb.arguments.some(arg => arg.role === role.role));
...
//         if (verb.arguments.some(function(arg) arg.role === role.role))];
        ...
        verbFinder: function (input, selection) {
            ...
            function addParses(verbPiece, argString, order) {
                ...
                for (let verb of verbs) {
                    ...
                    for (let subname of verbPatterns[verb.id]) {
        ...
        getRoleByDelimiter: function (delimiter, roles) {
            ...
            return roles.filter(role => role.delimiter.toLowerCase() === delimiter).map(role => role.role);
        ...
        argFinder: function argFinder(argString, verb, input, thisQuery) {
            ...
            let possibleDelimiterIndices = Object.keys(words).filter(i => this.hasDelimiter(words[i], verb.id)).map(i => +i);
//      [+i for (i in words) if (this.hasDelimiter(words[i], verb.id))];
            ...
            EACH_DI:
                for (let delimiterIndices of possibleDelimiterCombinations) {
                    ...
                    if (this.branching === "right" && lastIndex === words.length - 1 ||
                        ...
                        CmdUtils.deblog("maybe this is why I'm dead.");
                    ...
                    for (let i = 0, l = delimiterIndices.length; i < l; ++i) {
                        ...
                        for (var j = jmin; j <= jmax; j++) {
                            ...
                            for (var role of rolesForEachDelimiterCache[delim]) {
        ...
        substituteNormalizedArgs: function substituteNormalizedArgs(parse) {
            ...
            for (let role in parse.args) {
                ...
                for (let i in args) {
                    ...
                    for (let substitute of this.normalizeArgument(arg)) {
                        for (let baseParse of baseParses) {
        ...
        applyObjectsToOtherRoles: function applyObjectsToOtherRoles(parse) {
            ...
            if (parse._verb.id) {
                rolesToTry = {};
                for (let arg of parse._verb.arguments) {
                    if (arg.role !== 'object')
            ...
            for (let key in parse.args.object) {
                ...
                if (!object.fromSelection && parse._verb.id) {
                    let roles = parse._verb.arguments.map(arg => arg.role);
//        let roles = [arg.role for each (arg in parse._verb.arguments)];
            ...
            for (let parse of returnArr) {
                ...
                let signature = Object.keys(parse.args);
//      let signature = [role for (role in parse.args)];
        ...
        suggestVerb: function suggestVerb(parse, inputMatchesSomeVerb) {
            ...
            for (let roleArgArray of Object.values(parse.args)) {
        ...
        suggestArgs: function suggestArgs(parse) {
            ...
            for (let role in parse.args) {
                ...
                for (let verbArg of parse._verb.arguments) {
                        ...
                        for (let i in parse.args[role]) {
                            ...
                            if (suggestions) { //new
                                for (let suggestionId in suggestions) {
                                    for (let baseOrder of combinations) {
            ...
            for (let role of unfilledRoles) {
                let missingArg;
                for (let arg of parse._verb.arguments) if (arg.role === role) {
                    missingArg = arg;
                    break;
                }
                ...
                for (let defaultValue of defaultValues) {
                    ...
                    if (defaultValue.text) {
                        ...
                        for (let roleDesc of this.roles) {
            ...
            for (let role of unfilledRoles) {
                let newreturn = [];
                for (let defaultValue of myDefaultsCache[role]) {
                    for (let parseToReturn of returnArr) {
        ...
        detectNounType:
            function detectNounType(currentQuery, x, nounTypeIds, callback) {
                ...
                setTimeout(function detectNounType_cachedCallback() {
...
//!!!!!!!!!!!
                    ...
                    for (let s of suggs) s.nountypeId = id;
                    return suggs;
                };
                var thisParser = this;
                var myCallback = function detectNT_myCallback(suggestions, id, asyncFlag) {
                    var ids = (
                        id != null
                            ? [id]
                            : uncachedNounTypeIds.filter(id => thisParser._nounCache.getTime(x, id)
                    ...
                    for (let newSugg of suggestions) {
...
//        currentQuery.dump("running callback ("+callback.name+") now");
                ...
                if (uncachedNounTypeIds.length) {
                    setTimeout(function detectNounType_asyncDetect() {
                        var returnArray = [];
                        ...
                        for (let id of uncachedNounTypeIds) {
                            ...
                            for (let result of resultsFromSuggest) {
                                if (result.summary) {
                            ...
                            if (!dT.getRequestCount(x, id)) {
                                ...
                                if (!hadImmediateResults) {
                                    for (let parseId of dT.getParseIdsToCompleteForIds(x, [id])) {
                                        currentQuery._verbedParses[parseId].complete = true;
                                    }
                                    if (currentQuery._verbedParses.every(parse => parse.complete))
...
//
    ...
    ParseQuery.prototype = {
        dump: function PQ_dump(msg) {
            CmdUtils.deblog(this._idTime + ":" + (new Date - this._idTime), msg);
        },
...
//      self._lastAsync = new Date;
        ...
        _yieldingParse: function* PQ__yieldingParse() {
            ...
            for (var preParse of this._preParses) {
            ...
            if (this.selObj.text && this.selObj.text.length) {
                ...
                for (let parse of this._possibleParses) {
                    push.apply(this._possibleParses,
            ...
            var newVerbedParses = [];
            for (let parse of this._verbedParses) {
            ...
            for (let parseId in this._verbedParses) {
                ...
                // This parse doesn't have any arguments. Complete it now.
                    setTimeout(function completeNow(thisQuery, parse) {
                    ...
                    for (let x of arg) {
                        ...
                        let argText = x.input;
                        let ids = parse._verb.arguments.filter(verbArg => verbArg.role === role)
                            .map(verbArg => verbArg.nountype.id);
                        // let ids = [verbArg.nountype.id
                        //            for (verbArg of parse._verb.arguments)
            ...
            for (let {argText, nounTypeIds} of
        ...
        tryToCompleteParses: function PQ_tryToCompleteParses(argText, ids, asyncFlag) {
            ...
            for (let parseId of parseIdsToTryToComplete) {
        ...
        completeParse: function PQ_completeParse(thisParse) {
            ...
            var addedAny = false;
            for (let newParse of suggestions)
                addedAny = this.addIfGoodEnough("scored", newParse) || addedAny;

            if (this._verbedParses.every(parse => parse.complete))
                this.finishQuery();
        ...
        get hasResults() {
            return this.aggregateScoredParses().length > 0;
        },
        ...
        addIfGoodEnough: function PQ_addIfGoodEnough(parseClass, newParse) {
            ...
            let parseIds = parseCollection.map(parse => parse._id);
//    let parseIds = [parse._id for each (parse in parseCollection)];
            ...
            if (newBar > theBar) {
                ...
                parseCollection.sort((a, b) => b.maxScore - a.maxScore);
...
// (argText,nountypeId) pairs have been started or completed
    ...
    NounTypeDetectionTracker.prototype = {
        ...
        getParseIdsToCompleteForIds: function
            ...
            let returnHash = {};
            for (let id of ids)
                for (let parseId of this.getParseIdsToComplete(arg, id))
                    returnHash[+parseId] = true;
            return Object.keys(returnHash);
        ...
        getRequestCount: function DT_getRequestCount(x, id) {
            ...
            for (let i in this.detectionSpace) {
                ...
                for (let j in this.detectionSpace[i]) {
                    ...
                    for (let req of this.detectionSpace[i][j].outstandingRequests) {
    ...
    NounCache.prototype = {
        ...
        getBestSugg: function NC_getBestSugg(arg, id) {
            ...
            if (!this.cacheSpace[arg][id].setTime)
                return null;
            var bestSugg = null;
            for (var sugg of this.cacheSpace[arg][id].suggs)
        ...
        addSugg: function NC_addSugg(arg, id, newSugg) {
            ...
            for (let oldSugg of this.cacheSpace[arg][id].suggs) {
...
// the property {{{args}}} should be set individually afterwards.
    ...
    Parse.prototype = {
        ...
        get displayHtml() {
            ...
            for (let neededArg of this._verb.arguments) {
                ...
                label = ('<span class="' +
                    ...
                    '">' + label + "</span>");
                for (let parserRole of parser.roles)
        ...
        get displayHtmlDebug() {
            return Utils.escapeHtml(this._id) + ": " + this.displayHtml + " (" +
        ...
        execute: function (context) {
            return this._verb.execute(this.firstArgs, context);
        },

        // **{{{Parse#preview()}}}**
        //
        // creates the verb preview.
        preview: function (previewBlock, context) {
            this._verb.preview(previewBlock, this.firstArgs, context);
        },
        ...
        getArgsAndNounTypeIdsToCheck: function PP_getArgsAndNounTypeIdsToCheck() {
            ...
            for (let role in this.args) {
                ...
                for (let arg of this.args[role]) {
                    ...
                    let nounTypeIds = {};
                    for (let verbArg of this._verb.arguments) {
                        if (verbArg.role == role) {
        ...
        getRequestCount: function PP_getRequestCount() {
            ...
            for (let {argText, nounTypeIds} of
                ...
                for (let id of Object.values(nounTypeIds))
        ...
        allNounTypesDetectionHasCompleted: function
            ...
            for (let {argText, nounTypeIds} of argsAndNounTypeIdsToCheck) {
        ...
        get unfilledRoles() {
            return this._verb.id
                ? this._verb.arguments.filter(verbArg => !(verbArg.role in this.args)).map(verbArg => verbArg.role)
                // ? [verbArg.role
        ...
        copy: function PP_copy() {
            ...
            // we did.
            for (let role in this.args)
                ret.args[role] = this.args[role].map(sugg => ({__proto__: sugg}));
